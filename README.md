...
한국 유학을 고려하는 외국인 학생이 **영어로 질문하면**, 숫자 질문은 **SQL**로 공공데이터를 조회하고 규정 질문은 **RAG**로 정부 문서를 검색해 **출처와 함께** 답하는 의사결정 지원 시스템입니다. 벡터 검색이 구조적으로 할 수 없는 집계·순위·비교는 라우터가 SQL 경로로 보내 처리하며, 근거가 임계값에 못 미치면 **답변을 생성하지 않고 거부(Abstention)** 합니다. 비자·체류 규정은 틀린 답 하나가 사람을 위험에 빠뜨릴 수 있는 도메인이기 때문입니다. `SELECT * FROM Korea` 팀의 2026 BIGDATA-USC Conference Hackathon 프로젝트입니다.
...
> `app.py` 는 개발 편의상 `from mock import answer_question` 로 시작합니다. 실제 파이프라인을 붙이려면 `from src.pipeline import answer_question` 한 줄로 교체하면 됩니다 (동일한 함수 시그니처).
...
├── mock.py                # 프론트 개발용 목업 (당일 src.pipeline 으로 교체)
